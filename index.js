...
export default class GesturePassword extends Component {
  ...
  _renderArrows = () => this.state.arrows.map((arrow, index) => {
  ...
  _renderPoints = () => this.state.points.map((point, index) => (
    <Point
      key={'point-' + index}
  ...
  _renderLines = () => this.state.lines.map((line, index) => {
    if (this.state.isWarning) {
    ...
    if (point == null) {
      ...
      if (this._currentLine == null) {
        ...
        if (!this.props.allowCross) {
          let crossPoint = Utils.getCrossPoint(
            this.state.points,
            this._currentPoint,
            point,
            this._pointRadius,
